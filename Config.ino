...
String getUpdateUrl(){
 return "http://release.bhonofre.pt/release_cat_feeder.bin";
 }
String getHostname(){
...
void loadStoredConfiguration(){
    ...
    if(SPIFFS.exists(CONFIG_FILENAME)){
      ...
      if(cFile){
        ...
        if (storedConfig.success()) {
          ...
          configJson.set("hardwareId", String(ESP.getChipId()));
          configJson.set("type", "CAT_FEEDER");
  ...
  if(configFail){
    ...
    configJson.set("nodeId",String(HARDWARE) +"-"+String(ESP.getChipId()));
    configJson.set("homeAssistantAutoDiscoveryPrefix", "homeassistant");
    configJson.set("hostname",String(HARDWARE) +"-"+String(ESP.getChipId()));
